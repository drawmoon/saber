...
/**
 * A custom exception class that extends {@link SaberException}. This exception is used to handle
 * engine-related errors in the Saber system. It provides additional functionality for finding root
 * causes of a specific type within the exception chain.
 *
 * @author drash
 * @version 1.0
 * @since 2024
...
public class EngineException extends SaberException {
   ...
   *
   * @param cause the cause (which is saved for later retrieval by the getCause() method). A <code>
   *     null</code> value is permitted, and indicates that the cause is nonexistent or unknown.
   */
  ...
  /**
   * Finds a root cause of a given type within the exception chain, or returns <code>null</code> if
   * no root cause of that type was found.
   *
   * @param <T> the type of the root cause to find
   * @param type the class object representing the type of the root cause to find
   * @return the root cause of the specified type, or <code>null</code> if not found
   */
  @CheckForNull
  ...
  /**
   * Recursively searches through the exception chain to find a root cause of a given type, or
   * returns <code>null</code> if no root cause of that type was found.
   *
   * @param <T> the type of the root cause to find
   * @param t the throwable to start searching from
   * @param type the class object representing the type of the root cause to find
   * @return the root cause of the specified type, or <code>null</code> if not found
   */
  ...
  /**
   * Sets the maximum number of cause lookups when searching for a root cause of a specific type.
   *
   * @param maxCauseLookups the maximum number of cause lookups
   */
